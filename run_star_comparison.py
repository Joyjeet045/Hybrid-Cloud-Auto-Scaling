"""STAR Table 3 comparison harness for NF-DiagScale on the HGraphScale env.

Runs the NF-DiagScale controller on the 9 scenarios reported in the STAR paper
...
We report only what we actually measured: every NF-DiagScale cell is produced by
...
def main():
    ...
    results = {}
    print(f"\nNF-DiagScale vs STAR Table 3  (seeds={args.seeds}, budget=${BUDGET:.0f}/day)")
    print("=" * 92)
    print(f"{'Scenario':<9}{'AWS-Scale':>12}{'ProScale':>13}{'DeepScale':>13}"
          f"{'DRPC':>13}{'STAR':>12}{'NF-DiagScale':>16}")
    print("-" * 92)
    ...
    print("-" * 92)
    if total:
        print(f"NF-DiagScale beats STAR on {wins}/{total} STAR scenarios (lower MRT, Vio=0).")
